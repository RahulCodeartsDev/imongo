...
export class WizardPage implements OnInit {
  ...
  formValue = {phoneNumber: '', test: '',surveyQuestions:''};
  telForm: FormGroup;
  imgForm: FormGroup;
  tellForm: FormGroup;
  finform: FormGroup;
  ...
  currentSlide:any;
  router: any;
  constructor(private fb: FormBuilder, public navCtrl: NavController, private platform: Platform, private loadingController: LoadingController, private actionSheetCtrl: ActionSheetController, private _uploadService: ImageUploadService, public alertController: AlertController, private u_location: UserLocationService, private _userFormService: UserFormService) {
  ...
  createForms(): any {
    ...
    this.sur = new FormGroup({
      surveyQuestions: new FormControl({
        value: this.formValue.surveyQuestions
      }, )
    });
    ...
    this.tell2Form = new FormGroup({
      Tell2_us: new FormControl('',Validators.required)
    })
  ...
  prev(){
    ...
    if(this.currentSlide == 5){
      if(this.surveyans3!="" && this.surveyans3!=undefined){
  ...
  qnext(){
    ...
    if(this.currentSlide == 2){
      console.log(this.surveyans2)
      if(this.surveyans2!="" && this.surveyans2!=undefined){
        this.isNotValid= false;
      }
      else{
        this.isNotValid= true;
...
onRatingChange(){
  ...
  async loadFileData(fileNames: string[]){
    ...
    const fileData:any = this.images[0];
    const data = new FormData();
    data.append('file', fileData);
    data.append('upload_preset', 'geggzsit');
    data.append('cloud_name', 'dhgtuafmi');
...
onSurveySubmit(){
  ...
  //console.log(this.sur.value)

  
  
  if(this.currentSlide == 2){
    this.surveyans1 = this.sur.value.surveyQuestions
  }

  if(this.currentSlide == 3){
    this.surveyans2 = this.sur.value.surveyQuestions
  }

  if(this.currentSlide == 4){
    this.surveyans3 = this.sur.value.surveyQuestions
  }
}
...
async presentAlertMultipleButtons() {
  const alert = await this.alertController.create({
    ...
    // subHeader: 'Subtitle',
    message: 'If you leave then your selection may not save',
...
nextEnd() {
  ...
  this.u_location.getLocationService().then(resp=>{
    this.lang = resp.lng
    this.lat = resp.lat
    console.log('user longitude : '+ resp.lng);
    console.log('user latitude : '+ resp.lat);
  })
  this.userForm = new UserForm ();
  // console.log("Country Code: "+this.phoneNumber.value.dialCode);
  // console.log("Country iso Code: "+this.phoneNumber.value.isoCode);
  // console.log("International Phone Number: "+this.phoneNumber.value.internationalNumber);
  // console.log("National Phone Number: "+this.phoneNumber.value.nationalNumber);
  // console.log("Mango question 1 ans: "+this.surveyans1);
  // console.log("Mango question 2 ans: "+this.surveyans2);
  // console.log("Mango question 3 ans: "+this.surveyans3);
  // console.log("idea about our institute? ans:"+this.tellForm.value.Tell_us);
  // console.log("TELL US WHAT HAPPENED ans:"+this.tell2Form.value.Tell2_us);
  // console.log("OVERALL RATING ans:"+this.finform.value.Final);
  this.userForm.phoneNo = this.phoneNumber.value.dialCode
  this.userForm.Tell_us = this.tellForm.value.Tell_us
